                              ...
                              <tr key={match.id} className="hover:bg-gray-50/60 transition-colors">
                                ...
                                <td className="py-3.5 px-4 text-center">
                                  <Badge className={match.source === 'tor' ? 'bg-blue-50 text-blue-700 border-blue-200 text-xs' : 'bg-purple-50 text-purple-700 border-purple-200 text-xs'}>
                                    {match.source === 'tor' ? '≡ƒôä TOR' : '≡ƒÆ╝ Work Exp'}
                                  </Badge>
                                </td>
                                <td className="py-3.5 px-4 text-center">
              ...
              <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                ...
                {!isFinalized && (
                  <p className="text-xs text-gray-600 mt-2">
                    AI evaluation does not run automatically when reopening a finalized application. Click ΓÇ£Re-run AI EvaluationΓÇ¥ to display the latest subject matches.
                  </p>
                )}
              </div>
                                    ...
                                    <tr key={match.id} className="hover:bg-blue-50/40 transition-colors">
                                      ...
                                      <td className="py-3 px-3 align-top">
                                        {match.tor_subject ? (
                                          ...
                                          <div className="bg-purple-50/80 border border-purple-200 rounded-md p-2">
                                            <div className="font-semibold text-purple-900 text-xs">
                                              ≡ƒÆ╝ {match.work_experience.job_title}
                                            </div>
                                      ...
                                      <td className="py-3 px-2 align-middle text-center">
                                        <Badge variant="outline" className={`text-[11px] font-medium ${match.source === 'tor' ? 'border-amber-300 text-amber-800 bg-amber-50' : 'border-purple-300 text-purple-800 bg-purple-50'}`}>
                                          {match.source === 'tor' ? '≡ƒô£ TOR' : '≡ƒÆ╝ Work'}
                                        </Badge>
                                      </td>
                                      <td className="py-3 px-2 align-middle text-center">
                  ...
                  <div>
                    ...
                    {matchToRemove.tor_subject ? (
                      <span className="text-amber-900 font-medium">
                        ≡ƒô£ {matchToRemove.tor_subject.code} - {matchToRemove.tor_subject.title} ({matchToRemove.tor_subject.units}u, Grade: {matchToRemove.tor_subject.grade || 'N/A'})
                      </span>
                    ) : matchToRemove.work_experience ? (
                      <span className="text-purple-900 font-medium">
                        ≡ƒÆ╝ {matchToRemove.work_experience.job_title} at {matchToRemove.work_experience.company_name} ({matchToRemove.work_experience.years} yrs)
                      </span>
                    ) : (
                ...
                <div className="p-3 bg-red-50/70 border border-red-200 rounded-lg text-xs space-y-1.5">
                  ...
                  <div>
                    ...
                    {matchToReject.tor_subject ? (
                      <span className="text-amber-900 font-medium">
                        ≡ƒô£ {matchToReject.tor_subject.code} - {matchToReject.tor_subject.title} ({matchToReject.tor_subject.units}u, Grade: {matchToReject.tor_subject.grade || 'N/A'})
                      </span>
                    ) : matchToReject.work_experience ? (
                      <span className="text-purple-900 font-medium">
                        ≡ƒÆ╝ {matchToReject.work_experience.job_title} at {matchToReject.work_experience.company_name} ({matchToReject.work_experience.years}y)
                      </span>
                    ) : (
                      <span className="italic text-gray-500">None</span>
                    )}
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
            ...
            {match.tor_subject ? (
              <div className="text-xs text-gray-800">
                <span className="font-semibold text-amber-900">≡ƒô£ {match.tor_subject.code} - {match.tor_subject.title}</span>
              ...
              <div className="text-xs text-purple-900 font-medium">
                ≡ƒÆ╝ {match.work_experience.job_title} at {match.work_experience.company_name} ({match.work_experience.years}y)
              </div>
            ) : (
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              {hasTor && (
                <optgroup label="≡ƒô£ Applicant Scanned TOR Subjects">
                  {torSubjects.map((s) => {
                    ...
                    return (
                      ...
                      >
                        {isInsufficient ? 'ΓÜá∩╕Å ' : ''}{s.code} - {s.title} ({s.units}u) [Grade: {s.grade || 'N/A'}]{isInsufficient ? ` - [Insufficient: needs ${curUnits}u]` : ''}
                      </option>
                    );
                  })}
                </optgroup>
              ...
              {hasWork && (
                <optgroup label="≡ƒÆ╝ Applicant Work Experience (Work Crediting)" style={{ color: '#7e22ce', fontWeight: 'bold' }}>
                  {workExperiences.map((w) => (
                    <option key={w.id} value={`work:${w.id}`} style={{ color: '#7e22ce', backgroundColor: '#f3e8ff', fontWeight: '600' }}>
                      ≡ƒÆ╝ [Work] {w.job_title} - {w.company_name} ({w.years} yrs)
                    </option>
                  ))}
                </optgroup>
              )}
          ...
          <div className="flex flex-col gap-2">
            ...
            {hasTor || hasWork ? (
              ...
              >
                ...
                {hasTor && (
                  <optgroup label="≡ƒô£ Available TOR Subjects">
                    {torSubjects.map(subject => {
                      ...
                      return (
                        <option 
                          ...
                          disabled={isInsufficient}
                        >
                          {isInsufficient ? 'ΓÜá∩╕Å ' : ''}{subject.code} - {subject.title} ({subject.units}u) [{subject.grade}]{isInsufficient ? ` - [Insufficient: needs ${curriculum.units}u]` : ''}
                        </option>
                      );
                    })}
                  </optgroup>
                ...
                {hasWork && (
                  <optgroup label="≡ƒÆ╝ Applicant Work Experience (Work Crediting)" style={{ color: '#7e22ce', fontWeight: 'bold' }}>
                    {workExperiences.map(work => (
                      ...
                      >
                        ≡ƒÆ╝ [Work] {work.job_title} - {work.company_name} ({work.years} yrs)
                      </option>
                    ))}
                  </optgroup>
                )}
            ...
            {otherSubjects.map(subject => {
              ...
              return (
                <option key={subject.id} value={`tor:${subject.id}`} disabled={isInsufficient}>
                  {isInsufficient ? 'ΓÜá∩╕Å ' : ''}{subject.code} - {subject.title} ({subject.units}u) [{subject.grade}]{isInsufficient ? ` - [Insufficient: needs ${match.curriculum_subject.units}u]` : ''}
                </option>
              );
            })}
          </optgroup>
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <select className="border border-green-200 px-2 py-1 text-xs min-w-[280px] bg-green-50 rounded" value={selected} onChange={(e) => setSelected(e.target.value)}>
        ...
        {hasTor && (
          <optgroup label="≡ƒô£ TOR Subjects">
            {torSubjects.map(subject => {
              const isInsufficient = match.curriculum_subject && Number(subject.units || 0) < Number(match.curriculum_subject.units || 0);
              return (
                <option key={subject.id} value={`tor:${subject.id}`} disabled={isInsufficient}>
        ...
        {hasWork && (
          <optgroup label="≡ƒÆ╝ Work Experience" style={{ color: '#7e22ce', fontWeight: 'bold' }}>
            {workExperiences.map(work => (
        ...
        {hasTor && (
          <optgroup label="≡ƒô£ TOR Subjects">
            {passedSubjects.map(subject => {
              ...
              return (
                <option key={subject.id} value={`tor:${subject.id}`} disabled={isInsufficient}>
                  {isInsufficient ? 'ΓÜá∩╕Å ' : ''}{subject.code} - {subject.title} ({subject.units}u) {subject.grade ? `[${subject.grade}]` : ''}{isInsufficient ? ` - [Insufficient: needs ${match.curriculum_subject.units}u]` : ''}
                </option>
              );
            })}
          </optgroup>
